...
fn loader() {
    ...
    assert_eq!(
        None,
        wall.lookup("VertexlitGeneric.$detailtexturetransform")
    );
    assert_eq!(
        "models\\props_forest/cliff_wall_06",
        wall.lookup("VertexlitGeneric.$baseTexture")
            .unwrap()
            .as_str()
            .unwrap()
    );
    assert_eq!(
        "1",
        wall.lookup("VertexlitGeneric.$seamless_detail")
            .unwrap()
            .as_str()
            .unwrap()
    );

    let wall = vdf::load("tests/scout_red_invun_zombie.vdf").unwrap();

    assert_eq!(
        "models/player/scout/scout_red_zombie",
        wall.lookup("VertexLitGeneric.$basetexture")
            .unwrap()
            .as_str()
            .unwrap()
